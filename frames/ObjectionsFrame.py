...
class Objections_Frame(tk.CTkFrame):
    ...
    def redraw_all(self):
        ...
        self.params = []
        for opt in self.options:
            optbox = tk.CTkButton(master=self.list_frame, text_color=("black", "white"), text=opt, corner_radius=0,
                                  anchor="w", fg_color="transparent", hover=False,
                                  command=partial(self.master.toggle_objection, str(opt)))
