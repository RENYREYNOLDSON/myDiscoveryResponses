...
class Bar_Frame(tk.CTkFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        self.file = tk.CTkOptionMenu(master=self, anchor="center", variable=var, values=["New Window"], width=100,
                                     corner_radius=0, bg_color="transparent", command=self.call_file)
        ...
        self.file._dropdown_menu.add_command(label="Open Folder", command=self.master.select_folder,
                                             accelerator="Ctrl+F")
        ...
        self.file._dropdown_menu.add_command(label="Export Check With Client",
                                             command=self.master.export_check_with_clients)
        ...
        self.options = tk.CTkOptionMenu(master=self, anchor="center", variable=var,
                                        values=["Firm Details", "Objections", "Shortcuts", "Settings", "User Guide"],
                                        width=100, corner_radius=0, bg_color="transparent", command=self.call_options)
        ...
        # Details
        self.details = tk.CTkButton(master=self, text="File Details", fg_color="transparent", width=100,
                                    command=self.master.view_details, corner_radius=0)
        ...
        self.undo = tk.CTkButton(master=self, text="🡐", fg_color="transparent", font=arrow_font, width=40,
                                 command=self.master.undo_action, corner_radius=0)
        self.undo.pack(side="left")
        self.disable_undo()
        self.undo_tooltip = add_tooltip(self.undo, "Undo change (Ctrl-Z)")
        self.redo = tk.CTkButton(master=self, text="🡒", fg_color="transparent", font=arrow_font, width=40,
                                 command=self.master.redo_action, corner_radius=0)
        ...
        # Toggle Fullscreen
        self.fullscreen = tk.CTkButton(master=self, text="⤢", fg_color="transparent", width=40, corner_radius=0,
                                       command=self.master.toggle_fullscreen)
        self.fullscreen.pack(side="right")

        # Clear
        self.clear = tk.CTkButton(master=self, text="Clear", fg_color="transparent", width=100, corner_radius=0,
                                  command=self.master.clear)
        self.clear.pack(side="right")
        self.clear_tooltip = add_tooltip(self.clear, "Clear this request")

        # Copy Previous
        self.copy = tk.CTkButton(master=self, text="Copy Previous", fg_color="transparent", width=100,
                                 command=self.master.copy_previous, corner_radius=0)
        self.copy.pack(side="right")
        self.copy_tooltip = add_tooltip(self.copy, "Copy the previous request")

        # Preview
        self.preview = tk.CTkButton(master=self, text="Preview", fg_color="transparent", width=100, corner_radius=0,
                                    command=self.master.preview_text)
        self.preview.pack(side="right")
        self.preview_tooltip = add_tooltip(self.preview, "Preview current response only")

        # Save
        self.save = tk.CTkButton(master=self, text="Save", fg_color="transparent", width=100, corner_radius=0,
                                 command=self.master.quick_save)
        self.save.pack(side="right")
        self.save_tooltip = add_tooltip(self.save, "Save the current client (Ctrl-S)")

        # Language Text
        self.language_text = tk.CTkLabel(master=self, text=self.master.CONFIG["spelling"]["language"],
                                         text_color="grey")
