...
class Landing_Frame(tk.CTkFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        title = tk.CTkLabel(master=title_frame, text="  Software for the creation of Discovery Responses", font=font,
                            anchor="w")
        title.pack(fill="x")
        ...
        # Button List
        # New Client
        self.new_client_button = tk.CTkButton(master=button_frame, font=font, hover=False, text="➕ New Client",
                                              anchor="w", fg_color="transparent", text_color=("black", "white"),
                                              command=master.new_client)
        ...
        # Load File
        self.load_file_button = tk.CTkButton(master=button_frame, font=font, hover=False, text="📂 Load File",
                                             anchor="w", fg_color="transparent", text_color=("black", "white"),
        ...
        # User Guide
        self.user_guide_button = tk.CTkButton(master=button_frame, font=font, hover=False, text="📖 User Guide",
                                              anchor="w", fg_color="transparent", text_color=("black", "white"),
        ...
        # Settings
        self.settings_button = tk.CTkButton(master=button_frame, font=font, hover=False, text="⚙️ Settings", anchor="w",
                                            fg_color="transparent", text_color=("black", "white"),
                                            command=master.view_settings)
        self.settings_button.pack(pady=2, fill="x")
        self.settings_tooltip = add_tooltip(self.settings_button, "Open software settings")
        # Exit
        self.exit_button = tk.CTkButton(master=button_frame, font=font, hover=False, text="❌ Exit", anchor="w",
                                        fg_color="transparent", text_color=("black", "white"),
                                        command=master.exit_window)
        ...
        while i < min(8, len(master.RECENTS)):
            self.new_client_button = tk.CTkButton(master=self.recent_frame, hover=False, font=font,
                                                  text=master.RECENTS[i].split("/")[-1], anchor="w",
                                                  fg_color="transparent", text_color=("black", "white"),
                                                  command=partial(master.load, master.RECENTS[i]))
            self.new_client_button.pack(pady=2, fill="x")
            i += 1
    ...
    def update_recents(self, recents):
        ...
        while i < min(8, len(recents)):
            self.new_client_button = tk.CTkButton(master=self.recent_frame, hover=False, font=font,
                                                  text=recents[i].split("/")[-1], anchor="w", fg_color="transparent",
                                                  text_color=("black", "white"),
                                                  command=partial(self.master.load, recents[i]))
            self.new_client_button.pack(pady=2, fill="x")
            i += 1
