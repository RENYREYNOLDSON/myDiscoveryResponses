...
class EditObjections(tk.CTkToplevel):
    ...
    def __init__(self, master):
        ...
        # Save
        save_btn = tk.CTkButton(master=self.bar_frame, fg_color="transparent", corner_radius=0, text="Save All",
                                width=100, command=self.save_all)
        save_btn.pack(side="left")
        # Add
        new_btn = tk.CTkButton(master=self.bar_frame, fg_color="transparent", corner_radius=0, text="New", width=100,
                               command=self.new)
        new_btn.pack(side="left")
        # Delete
        del_btn = tk.CTkButton(master=self.bar_frame, fg_color="transparent", corner_radius=0, text="Delete", width=100,
                               command=self.delete)
        del_btn.pack(side="left")
        # Reset all
        reset_btn = tk.CTkButton(master=self.bar_frame, fg_color="transparent", corner_radius=0, text="Reset All",
                                 width=100, command=self.reset_all)
        ...
        self.objection_entry = SmartTextbox(main_master=self.master, master=self.edit_frame, wrap="word", height=100,
                                            font=font, text_color=text_color, fg_color=bg_color)
        ...
        self.additional_entry = SmartTextbox(main_master=self.master, master=self.edit_frame, wrap="word", height=50,
                                             font=font, text_color=text_color, fg_color=bg_color)
        ...
        self.autofill_entry = tk.CTkTextbox(master=self.edit_frame, wrap="word", height=50, font=font,
    ...
    def redraw_buttons(self):
        ...
        for obj in self.objections:
            ...
            self.objection_buttons.append(
                tk.CTkButton(master=button_frame, text_color=("black", "white"), fg_color="transparent", text=obj,
                             anchor="w", command=partial(self.load_objection, obj)))
            self.objection_buttons[-1].pack(anchor="w", fill="x", padx=(5, 0), side="left", expand=True)

            up_arrow = tk.CTkButton(master=button_frame, text_color=("black", "white"), fg_color="transparent",
                                    text="▲", anchor="w", command=partial(self.up_button, c), width=20)
            up_arrow.pack(side="right")
            down_arrow = tk.CTkButton(master=button_frame, text_color=("black", "white"), fg_color="transparent",
                                      text="▼", anchor="w", command=partial(self.down_button, c), width=20)
    ...
    def save_all(self):
        if self.current_objection != "":
            ...
            self.objections = {key if key != self.current_objection else name: value for key, value in
                               self.objections.items()}
