...
HIGHLIGHT_WORDS = ["photograph", "videotape", "document", "evidence", "property damage", "lost wages", "injury",
                   "injuries"]
...
def spellcheck(obj, text):
    ...
    for err in chkr:  # Checks all of the matches and adds them to issues
        if err.word not in ignores:
            ...
            obj.tag_add("spelling", "0.0 + " + str(err.wordpos - 1) + " chars",
                        "0.0 + " + str(len(err.word) + err.wordpos - 1) + " chars", )
            new_issue = ["Spelling Issue", "Always Ignore", err.word, err.wordpos, len(err.word),
                         suggestions[:5]]  # Only get 1st five
