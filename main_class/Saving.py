...
class Saving:
    ...
    def select_save_file(self):
        ...
        filename = tk.filedialog.asksaveasfilename(title="Save Current File", filetypes=(
        ("Discovery Save File", "*.discovery"), ('All files', '*.*'))).replace(".discovery", "")
        if filename == "":
            return
    ...
    def select_save_client(self):
        # Select a folder and save name
        filename = tk.filedialog.asksaveasfilename(title="Save Current Client", filetypes=(
        ("Discovery Save File", "*.discovery"), ('All files', '*.*'))).replace(".discovery", "")
        if filename == "":
            return
    ...
    def save_client(self, filename):
        ...
        try:
            ...
            msg = CTkMessagebox(title="Saving Issue",
                                message="The selected file: " + str(
                                    filename) + " could not be saved [2]! SAVE UNDER A NEW NAME!",
                                icon="warning",
                                option_1="Okay",
                                corner_radius=0,
                                sound=True,
    ...
    def load_client_feedback(self):
        if self.file_open():
            filename = tk.filedialog.askopenfilename(title='Load Client Feedback',
                                                     filetypes=(('Word Docx', '.docx'), ('All files', '*.*')))
            if filename != "":
                            ...
                            for req in file.reqs:
                                if str(req.custom_key) == f["key"]:
                                    if req.req_type == "RFA" and req.RFA_option != "Custom":  # If RFA then set this to custom
                                        ...
                                        req.resp = ""
                                    req.resp = req.resp + f[
                                        "response"]  # Add client feedback to end of the response text
