...
class Requests_Frame(tk.CTkFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        # Client Button
        self.add_client_button = tk.CTkButton(master=client_title_frame, width=20, height=10, font=("arial", 20),
                                              text="+", fg_color="transparent", text_color=("black", "white"),
                                              hover=False, command=master.new_client)
        ...
        text.pack(side="left", fill="both", padx=10)
        self.add_file_button = tk.CTkButton(master=file_title_frame, width=20, height=10, font=("arial", 20), text="+",
                                            fg_color="transparent", text_color=("black", "white"), hover=False,
                                            command=master.select_file)
    ...
    def show_clients(self, clients):
        ...
        for i in clients:
            ...
            if i.saved == False:
                saved_bit = "⦁ "
                unsaved += 1
            button = tk.CTkButton(master=self.clients_frame, image=CLIENT_ICON, anchor="w",
                                  text=saved_bit + get_name(i.name, 22), hover=False, corner_radius=0,
                                  fg_color=fg_color, command=i.set, text_color=i.color)
        ...
        for i in clients:
            ...
            if i == self.master.current_client:
                self.client_buttons[c].configure(fg_color='#144870', text_color=i.color,
                                                 text=saved_bit + get_name(i.name, 22))
            else:
    ...
    def show_files(self, files):
        ...
        for i in files:
            ...
            if i == self.master.current_client.current_file:
                fg_color = '#144870'
            button = tk.CTkButton(master=self.file_frame, image=PDF_ICON, anchor="w", text=get_name(i.name, 22),
                                  hover=False, corner_radius=0, fg_color=fg_color, command=i.set, text_color=i.color)
            button.pack(fill="x", side="top")
            if self.master.CONFIG["general"]["hover_tooltips"]:
                add_tooltip(button, i.name, wraplength=2000)
    ...
    def scroll_to_file(self, reset=False):
        if self.master.current_client != "":
            ...
            current_index = self.master.current_client.files.index(
                self.master.current_client.current_file)  # Index of current request
    ...
    def show_list(self, reqs, req_type="RFA"):
        ...
        for i in reqs:
            ...
            if i.custom_key == "":
                ...
                text = i.custom_key
            button = tk.CTkButton(master=self.list_frame, text=req_type + " NO. " + str(text), corner_radius=0,
                                  text_color=color, fg_color=fg_color, hover=False, command=i.set)
            button.pack(fill="x", side="top")
