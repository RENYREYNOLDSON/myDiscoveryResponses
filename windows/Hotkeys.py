...
class Hotkeys(tk.CTkToplevel):
    ...
    def redraw_hotkeys(self):
        ...
        for hot in self.hotkey_array:
            ...
            # Add Delete Button
            delete = tk.CTkButton(master=frame, text="🗑", text_color=("black", "white"), fg_color="transparent",
                                  hover=False, font=("Segoe UI", 20), width=40, command=partial(self.delete, count))
            delete.pack(side="left", padx=5)
            count += 1
        # Add New Button
        add_button = tk.CTkButton(master=self.scroll_frame, text="➕", width=40, fg_color="transparent",
                                  command=self.add, text_color=("black", "white"))
        add_button.pack()
